...
if (empty($request['staff_pass']) || empty($request['staff_pass_confirm'])) {
    $errors ['未入力'] = 'パスワードが入力されていません。';
}

if (!preg_match('/\A(?=.*?[a-z])(?=.*?\d)[a-z\d]{8,20}+\z/i', $request['staff_pass'])
      ||  !preg_match('/\A(?=.*?[a-z])(?=.*?\d)[a-z\d]{8,20}+\z/i', $request['staff_pass_confirm'])) {
  $errors['規則性違反'] = 'パスワードは８文字以上20文字以下に英数字を最低１文字含むようにしてください。';
}

if ($request['staff_pass'] !== $request['staff_pass_confirm']){
  $errors['不一致'] = 'パスワードとパスワード確認が一致しません。';
}

// エラーがある場合、セッションにエラーを保存してview_reset_form.phpを読み込む
if (!empty($errors)) {
  $_SESSION['errors'] = $errors;
  $_SESSION['password_reset_token'] = $request['password_reset_token']; // トークンをセッションに保存
  require 'view_reset_form.php';
  exit();
}
