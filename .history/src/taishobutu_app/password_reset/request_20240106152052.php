...
try {
  ...
  $stmt = $db_host->prepare($sql);
  $stmt->bindValue(':mail_address',$mail_address, PDO::PARAM_STR);
  $stmt->bindValue(':token', $passwordResetFiredeptstaff->token, PDO::PARAM_STR);
  ...
  //URLの設定
  $url = "http://localhost:50080/taishobutu_app/password_reset/reset_form.php?token={$passwordResetToken}";
