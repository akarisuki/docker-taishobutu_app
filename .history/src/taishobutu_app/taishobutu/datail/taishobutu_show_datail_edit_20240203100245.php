...
foreach ($result_taishobutu_datail as $column_name => $value) {
  if ($value) {
      $displayData[$column_name] = $value;
      $operation[$column_name] = 'update';
  } else {
      $displayData[$column_name] = '';
      $operation[$column_name] = 'insert';
  }
}
...
<head>
    ...
    <script src="../../common/script/datail/taishobutu_datail_script.js"></script>
    ...
    <form method="POST" action="taishobutu_show_datail_edit_done.php">
      ...
      <div class="CapacityWindowlessFloorBuildingStructureCellSet">
        <div class="WindowlessFloorCellSet">
          <div class="WindowlessFloorValueCell"><input type="text" name="windowless_floor" id="windowless_floor" value="<?php echo $displayData['windowless_floor'];?>"></div>
        ...
        <div class="BuildingStructureValueCell">
            <select name="building_structure" id="building_structure">
                <option value="木造" <?php echo ($displayData['building_structure'] == '木造') ? 'selected' : ''; ?>>木造</option>
                <option value="RC造" <?php echo ($displayData['building_structure'] == 'RC造') ? 'selected' : ''; ?>>RC造</option>
                <option value="SRC造" <?php echo ($displayData['building_structure'] == 'SRC造') ? 'selected' : ''; ?>>SRC造</option>
                <option value="S造" <?php echo ($displayData['building_structure'] == 'S造') ? 'selected' : ''; ?>>S造</option>
            </select>
        </div>
      ...
      <div class="BuildingInfoCellSet">
        ...
        <div class="BuildingInfoMiddleCellCet">
          <div class="BuildingClassificationCellSet">
            <div class="BuildingClassificationValueCell"><input type="text" name="building_classification" id="building_classification" value="<?php echo $displayData['building_classification'];?>"></div>
            <div class="BuildingClassificationCell"><h6>建築物区分</h6></div>
          </div>
          <div class="InteriorLimitCellSet">
            <div class="InteriorLimitValueCell"><input type="text" name="interior_limit" id="interior_limit"  value="<?php echo $displayData['interior_limit'];?>"></div>
            <div class="InteriorLimitCell"><h6>内装制限</h6></div>
          </div>
          <div class="MainStructureCellSet">
            <div class="MainStructureValueCellSet"><input type="text" name="main_structure" id="main_structure"  value="<?php echo $displayData['main_structure'];?>"></div>
            <div class="MainStructureCell"><h6>主要構造</h6></div>
          </div>
        </div>
        <div class="BuildingInfoTopCellSet">
          <div class="FloorsCellSet">
            <div class="FloorsValueCell"><input type="text" name="floor" id="floor" value="<?php echo $displayData['floor'];?>"></div>
            <div class="FloorsCell"><h6>階数</h6></div>
          </div>
          <div class="NewConstructionDateCellSet">
            <div class="NewConstructionDateValueCellSet"><input type="text" name="new_construction_date" id="new_construction_date" value="<?php echo $displayData['new_construction_date'];?>"></div>
      ...
      <div class="OwnerCellSet">
        ...
        <div class="OwnerAddressCellSet">
          <div class="OwnerAddressValueCell"><input type="text" name="owners_address" id="owners_address" value="<?php echo $displayData['owners_address'];?>"></div>
      ...
      <?php foreach($operation as $key => $value) {
          echo '<input type="hidden" name="operation[' . htmlspecialchars($key) . ']" value="' . htmlspecialchars($value) . '">';
      }?>

      <button type="submit" class="button_submit"id="add_submit">更新</button>
    </form>
  </div>
</body>
