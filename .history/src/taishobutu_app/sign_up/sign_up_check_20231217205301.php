...
require_once __DIR__ . '/../common/db_operation/db_connect.php';
    ...
    if (empty($security_question)) {
        $error_security['質問未入力'] = '秘密の質問が選択されていません。';
    }
    
    if (empty($security_answer)) {
        $error_security['答え未入力'] = '秘密の質問の答えが入力されていません。';
    }
    


    $error = array_merge($error_name, $error_pass,$error_pass_confirm,$error_fire_dept_code, $error_security);
    ...
    if (empty($error)) {
        ...
        echo '<input type="hidden" name="security_question" value="' .  htmlspecialchars($security_question, ENT_QUOTES, 'UTF-8') . '">';
        echo '<input type="hidden" name="security_answer" value="' .  htmlspecialchars($security_answer, ENT_QUOTES, 'UTF-8') . '">';
        echo '</form>';
        echo '</body>';
    } else {
