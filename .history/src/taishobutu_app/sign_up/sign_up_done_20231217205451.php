<?php
    try{
            ...
            $securiy_question = $post['security_question'];
            $securiy_answer = $post['security_answer'];
            //パスワードの暗号化
            $hash_pass = password_hash($staff_pass, PASSWORD_DEFAULT);

            $sql = 'INSERT INTO firedept_staff SET staff_name = :staff_name,staff_pass = :staff_pass ,fire_dept_code = :fire_dept_code,securiy_question = :securiy_question,securiy';
            ...
            $stmt->bindValue(':securiy_question', $securiy_question,PDO::PARAM_STR);
            $stmt->execute();
            $_SESSION['flash'] = [
                'type' => 'success',
                'message' => '登録が完了しました。'
